...
public class Longevity_Module {
	...
	public void Admin_Nav_to_div() throws InterruptedException {
		...
		Thread.sleep(4000);
		driver.findElement(By.linkText("Longevity Management")).click();
		Thread.sleep(4000);
		
	}
	@Test
	public void Add_Longevity() throws InterruptedException {
	...
	Thread.sleep(4000);
	driver.findElement(By.xpath("//input[@placeholder=\"Enter longevity name\"]")).sendKeys("Longevity-21");
	driver.findElement(By.xpath("/html/body/div/div/div[2]/div/div[2]/div/form/div[5]/button[2]")).click();
	String tittle = driver.getTitle();
	Thread.sleep(4000);
	Assert.assertEquals(tittle, "Longevity Management | pt.Track");
	}
	
	@Test
	public void Edit_longevity() throws InterruptedException {
		Thread.sleep(4000);
		...
		Thread.sleep(4000);
		driver.findElement(By.xpath("//input[@placeholder=\"Enter longevity name\"]")).sendKeys("Longevity_001");
		Thread.sleep(4000);
	...
	@Test
	public void Archive_longevity() throws InterruptedException{
	...
	@Test
	public void Search_longevity() throws InterruptedException{
		...
		for (WebElement searchby : searchBy) {
			if (searchby.getText().equals("Updated by")) {
		...
		driver.findElement(By.id("searchText")).sendKeys("Salmina");
		Thread.sleep(4000);
		...
		for (int i=0; i<results.size(); i++) {
			...
			if (results.get(i).getText().contains("Salmina")) {
	...
	static String sort;
	@Test
	public void List_longevity() throws InterruptedException{
		try 
		{
		...
		List<WebElement> longevityColums = driver.findElements(By.xpath("//tbody[@class=\"ant-table-tbody\"]//tr/td[1]"));
		String[] beforeSort = new String[longevityColums.size()];
		for(int i=0; i<longevityColums.size(); i++) {
			beforeSort[i] = longevityColums.get(i).getText().trim();
		...
		Thread.sleep(4000);
		longevityColums = driver.findElements(By.xpath("//tbody[@class=\"ant-table-tbody\"]//tr/td[1]"));
		String[] AfterSort = new String[longevityColums.size()];
		for(int i=0; i<longevityColums.size(); i++) {
			AfterSort[i] = longevityColums.get(i).getText().trim();
